...
```powershell
$Url      = 'https://raw.githubusercontent.com/schutzz/toyotamahime/k8-bootstrap-v6/bootstrap/Start-Study01.ps1'
$Dest     = Join-Path $env:TEMP 'Start-Study01.ps1'
$Expected = '74a421190f50ed15e6febc7ae4dbfa77397d26f349b8a5f4b2a87e8dd1d19635'
...
The tag `k8-bootstrap-v6` points at a specific commit in this repository's history, the same way §4.1 pins Amenonuboco by tag rather than by a moving branch. `Start-Study01.ps1` also checks out that same tag by default when it clones Toyotamahime (its `-Ref` parameter defaults to `k8-bootstrap-v6`), so the commit you fetched this script from and the commit your attempt actually reproduces are the same one, even if `main` has moved on by the time you run this. If you would rather read the script before running it, it is right there in the repository you are about to clone: [`bootstrap/Start-Study01.ps1`](../bootstrap/Start-Study01.ps1).

...
[`provenance.json`](./provenance.json) is the machine-readable record: for each of the 72 extracted files it gives the Toyotamahime path, the Kakuriyo path, the **Kakuriyo blob id**, the SHA-256, the byte count, and whether the file carries CRLF.
...
The shipped `study01` package is therefore Kakuriyo's minus `gate_b2.py`, plus `scripts/tests/test_preflight_amend005_provisioning.py` (new; not present at the original `8873aee` export). Every file that *is* shipped is byte-identical to its Kakuriyo counterpart, most recently re-verified at `106e76d` (originally exported at `8873aee`; besides the new test file, only `protocol/amendments.md`, `protocol/c2-dnp3-range-derivation.md`, `protocol/dependencies.md`, `scripts/study01/frozen/apparatus.py`, and `scripts/study01/preflight.py` changed between the two, all independently re-diffed file-by-file against `provenance.json` before this baseline was cut).

...
Layer C runs the full lifecycle twice — once ending in `Stop-K8.ps1 -Success`, once ending in `Stop-K8.ps1` (Failed) — asserting in both cases that `final-status.json` is correct, `manifest.sha256` exists, the archive's SHA-256 matches its actual bytes, `transcript.txt` exists and is judged *complete* (not merely present; see below), and that every `steps.jsonl` entry's `step_index` is sequential and its `raw_output` artifact under `steps-raw/` actually exists — the process-independent evidence that step-level completeness is now proven from.

**Formal evidence authority: `steps.jsonl` + `steps-raw/`, not `transcript.txt`.** An earlier revision of `Invoke-K8Step` re-attached to `transcript.txt` with `Start-Transcript -Append` across process boundaries and relied on that mechanism's nested-session behaviour for completeness; an independent review found that behaviour was only empirically confirmed on this repository's PowerShell 7.6.6 test runtime, not on 7.6.2 (the version the original failed formal attempt, `k8-repro-20260914-001`, actually ran), and is not part of `Stop-Transcript`'s documented contract. `Invoke-K8Step` now writes each step's combined stdout/stderr straight to its own `steps-raw/NNNN.log` via `Add-Content` — the same process-independent file-append primitive `steps.jsonl` has always used, with no PowerShell transcription API involved. `transcript.txt` is still opened at bootstrap and still best-effort re-attached to by each step, and remains a retained artifact, but nothing about step-level evidence *completeness* depends on it any more; this requires no PowerShell version beyond the file's existing `#requires -Version 7.0`.

**`transcript.txt` completeness, checked explicitly instead of assumed.** `docs/k8-independent-reproduction-plan.md` section 10's Gate K8 exit criteria separately require a "complete transcript", not merely a present one — and because `Invoke-K8Step`'s re-attach is best-effort and silently swallows a failed `Start-Transcript -Append`, `transcript.txt` can in principle exist but stop covering steps partway through an attempt, with nothing about step-level formal evidence (`steps.jsonl` / `steps-raw/`) revealing that on its own. `Complete-K8Attempt` now calls `Test-K8TranscriptCoverage`, which counts each step's own `=== K8 STEP: ... ===` `Write-Host` banner inside the closed `transcript.txt` and compares that count against `steps.jsonl`'s own step count — a judgement made purely from the two artifacts' own text after the fact, not by inspecting live transcript/session state, so it does not reintroduce a dependency on `Start-`/`Stop-Transcript`'s nested behaviour. The result is written to `final-status.json` as `transcript_complete` (plus `transcript_step_markers`, `transcript_expected_steps`, and `transcript_completeness_reason`) unconditionally, on both success and failure closes, so an incomplete transcript is always an explicit, visible `false`, never a silent gap a reviewer has to notice on their own. Layer C asserts `transcript_complete -eq $true` on both lifecycle passes; Layer A separately regresses the `false` case by corrupting the transcript path mid-attempt and confirming `steps.jsonl` / `steps-raw/` stay fully intact regardless.

**Apparatus-integrity count self-consistency.** After the success-lifecycle pass, a further Runbook-layer check locates the `apparatus integrity test` entry in that pass's own `steps.jsonl`, reads its `raw_output` artifact (not `transcript.txt`), and structurally parses pytest's own summary line — per-status token by token (`passed`, `failed`, `error`/`errors`, `skipped`, `xfailed`, `xpassed`, `deselected`, the two-word `subtests passed`, and `warning`/`warnings`), anchored on the line's trailing `in <N>s` marker rather than assuming any one status word sits next to it — then cross-checks `passed` against the number `Study01/README.md` §3.1 states in its "NN tests should pass" sentence (read only from the prose between the `apparatus-check` and `apparatus-check-via-harness` markers, not a whole-file string match) and asserts every failure-shaped status is zero. A future change to the shipped test suite that adds or removes tests without updating that sentence in the same commit fails certification here, instead of surfacing only during a formal K8-3 attempt — which is exactly how this drifted from 69 to 74 tests unnoticed once (`AMEND-004` added 5 tests; see `evidence/reproduction/k8-repro-20260914-001/` in Kakuriyo for the independent review that found it). A second independent review of that first fix found its original whole-line regex (assuming `passed` sat immediately before `in Ns`) failed to parse summaries containing `skipped`/`xfailed`/`deselected` segments; the structural per-token parse above (`ConvertFrom-K8PytestSummary`) replaces it. A third independent review, confirmed against a live pytest run, found that same parser treated pytest's own `N warning(s)` annotation — not a test outcome — as an unrecognized segment, which would fail a genuinely all-passing run for an unrelated reason; `warning`/`warnings` is now tracked as its own informational count (never folded into `passed`, never gated on), the same way `subtests_passed` already was.

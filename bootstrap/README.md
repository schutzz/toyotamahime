...
`-Ref` defaults to this script's own release tag (`k8-bootstrap-v3` as of this writing, see the v3 changelog note in the script's own header), not a moving branch, so the commit `git clone` actually fetches is the same commit that was package-certified — even if the default branch has advanced by the time a VM attempt runs. See `docs/k8-packaging-certification.md`.
